...
def write_dict_to_csv(fasta_dict, output_file):
    ...
    with open(output_file, 'w', newline='') as csvfile:
        ...
        writer.writerow(['Sequence_ID', 'Sequence_Length', 'GC_Content (%)', 'Validation_Status'])
        for seq_id, seq in fasta_dict.items():
            seq_length = len(seq)
            gc_content = calculate_gc_content(seq)
            is_valid, message = validate_seq(seq)
            writer.writerow([seq_id, seq_length, f"{gc_content:.2f}", 'Valid' if is_valid else 'Invalid'])